...
def solution(files):
    ...
    # NUMBER : 한 글자에서 최대 다섯 글자
    # TAIL : 나머지 부분
    
    # 1. HEAD 부분을 기준으로 사전 순 정렬
    # 2. NUMBER 숫자 순으로 정렬 (숫자 앞 0 무시)
    # 3. HEAD, NUMBER가 같을 경우 원래 입력에 주어진 순서 유지
...
// 불을 키는 왼쪽 전구가 원하는 전구 상태인 지 봐야한다. (왼쪽 -> 오른쪽 이동하면 더 이상 확인할 수 없기 때문에.)
...
const now = input[1].split("").map((v) => Number(v));
now.splice(N, 1);
const copy = JSON.parse(JSON.stringify(now));

const next = input[2].split("").map((v) => Number(v));

const clickResult = Click(now);
const clickNonResult = NonClick(copy);

if (clickResult >= 0 && clickNonResult >= 0) {
    if (clickResult > clickNonResult) {
        console.log(clickNonResult);
    } else {
        console.log(clickResult);
    }
} else if (clickResult >= 0 && clickNonResult < 0) {
    console.log(clickResult);
} else if (clickResult < 0 && clickNonResult >= 0) {
    console.log(clickNonResult);
} else {
